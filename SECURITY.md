...
- 网关只读 CC Switch 配置，供应商环境变量可能进入进程内存；不通过诊断接口输出，不持久化。上游认证由 CC Switch 负责。
- 附件以随机 ID 落盘（文件名不参与拼路径）、临时文件原子替换、归档类型直接拒绝、总额配额限制
- 日志对常见凭据格式脱敏并省略 URL 查询参数；发布诊断前仍应人工检查错误正文中的个人数据。
- `/status/ccswitch` 仅返回明确允许的诊断字段；不返回令牌或完整供应商环境。
- Origin 白名单同时检查预检和实际请求。无 Origin 的本机客户端仍可使用非空占位令牌；多人共机场景建议设置固定 `EDGE_TOKEN`。
